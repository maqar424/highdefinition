...
def handle_update_gallery(event):
    ...
    if 'tags'         in body: updates['Tags']         = [t.strip() for t in (body['tags'] or []) if t and t.strip()]
    if 'startDate'    in body: updates['StartDate']    = (body['startDate'] or '').strip()
    if 'endDate'      in body: updates['EndDate']      = (body['endDate'] or '').strip()
